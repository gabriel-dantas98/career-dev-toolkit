...
def test_yearless_period_never_borrows_another_records_year() -> None:
    ...
    timeline = build_timeline((unresolved, dated), reference_year=2099)
...
def test_yearless_period_uses_only_its_own_observed_at_year() -> None:
    ...
    card = build_timeline((record,), reference_year=2099).cards[0]
...
def test_all_unresolved_yearless_inputs_are_explicit_and_do_not_crash() -> None:
    ...
    packet = build_promo_packet(records, reference_year=2099)
    timeline = build_timeline(records, reference_year=2099)
